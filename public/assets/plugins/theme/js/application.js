...
/* ---------------------------------------------------
	...
	function getInputData(formName) {
		...
		$.each($('.data-'+formName), function(index, item) {
			value = $.trim($(item).val());
			if(value!=null) {
				lbl			= $(item).attr('id').replace(formName+'-', '');
				data[lbl]	= value;
			}
		});
	...
	function validateRequiredFields(formName) {
		...
		$.each(filter, function(index, item) {
			...
			if(element.val()=='' && element.is(':visible')) {
				...
				$('.alert').remove();
				element.parents('.box-inner').prepend($('<div class="alert alert-danger">Falta ingresar información: <b>'+element.attr('data-description')+'</b></div>'));
				element.focus();
			}
		});
